...
export default function EgresosPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const allEgresos = await egresosRes.json()
      const docentesData = await docentesRes.json()
      setDocentes(docentesData)
      
      console.log("Current user ID:", user?.id)
      console.log("All egresos:", allEgresos)
      console.log("Docentes:", docentesData)
      
      // Filter egresos by current teacher ID - try multiple matching strategies
      const teacherEgresos = allEgresos.filter((egreso: Egreso) => {
        // Strategy 1: Direct match by docente field if it contains ID
        if (typeof egreso.docente === 'number' && egreso.docente === user?.id) {
          return true
        }
        
        // Strategy 2: Match by teacher name then get ID
        if (typeof egreso.docente === 'string') {
          const docente = docentesData.find((d: Docente) => 
            d.nombres === egreso.docente || 
            d.nombres?.toLowerCase() === egreso.docente?.toLowerCase()
          )
          console.log("Match found:", docente, "for egreso:", egreso.docente)
      ...
      console.log("Filtered egresos:", teacherEgresos)
      setEgresos(teacherEgresos)
    } catch {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* TOTAL CARD MOVED TO TOP RIGHT */}
        <div className="flex-shrink-0">
          <Card className="">
            <CardContent className="px-6">
              <div className="flex items-center justify-between gap-2">
                <div>
                  <p className="text-sm font-bold text-muted-foreground uppercase tracking-wider">TOTAL PAGOS RECIBIDOS</p>
                  <p className="text-xl font-black mt-1 text-primary">S/ {totalEgresos.toFixed(2)}</p>
                </div>
          ...
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow className="border-b border-border/50 hover:bg-transparent">
                ...
                <TableHead className="font-bold text-xs">Curso</TableHead>
                ...
                <TableRow key={e.idEgreso} className="border-b border-border hover:bg-muted/70 transition-colors">
                  ...
                  <TableCell className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-foreground leading-none">{e.curso}</span>
                    </div>
                  </TableCell>
                  <TableCell className="px-6 py-4">
